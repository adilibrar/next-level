...
function AvAccStock(){
    ...
    const[firstcountries,setFirstCountries]=useState([]);
    ...
    const getCountries = async()=>{
        try{
            const response= await axios.get(AppURL.StockDetail)
            setFirstCountries(response.data);
            //setFilteredCountries(response.data)
            const acc=firstcountries.filter(country=>{
                return country.item['type']['id'] != '1';
            });
            
                setCountries(acc);
                setFilteredCountries(acc);
                setLoading(false); 
    ...
    useEffect(()=>{
        axios.get(AppURL.NewStockType).then(response=>{
        const FilterAccData=()=>{
            const acc=response.data.filter(country=>{
                return country.type == 'Accessory';
            });
            setCountries(acc);
            setFilteredCountries(acc);
            setLoading(false); 
...
//console.log(filteredCountries);
    ...
    //const acc=countries.filter(item['type']['id'] != '1')
      // console.log('sa'); 
if(loading){
        return<>
