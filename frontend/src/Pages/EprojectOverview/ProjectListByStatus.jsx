import React, { Component, Fragment ,useEffect,useState} from 'react'
import NavMenu from '../../Componenets/Common/NavMenu'
import { useLocation } from 'react-router-dom'
import Table from 'react-bootstrap/Table';
import axios from 'axios'
import AppURL from '../../api/AppURL';
import DataTable from 'react-data-table-component';
//  components

import { Container,
  Card,

  Row,
  Col,
} from 'react-bootstrap';
// core components

import {Link, useNavigate} from 'react-router-dom';
function ProjectListByStatus() {
  const navigate =useNavigate();
  const[projects,setprojects]=useState([]);
  const location = useLocation();
  const check_login=sessionStorage.getItem('login');
  const getexternal=sessionStorage.getItem('external');
  const status=location.state.status;
  const [buttonClicked, setButtonClicked] = useState(true);
  const[countries,setCountries]=useState([]);
  const[search,setSearch]=useState([]);
  const[filteredCountries,setFilteredCountries]=useState([]);
  const[managers,setManagers]=useState([]);
  const usercode=sessionStorage.getItem('code');
  const[pmfilter,setpmfilter]=useState(false);
  useEffect(()=>{
      if(!check_login){
          navigate('/login')
        }
        if(usercode=='SSA'){
          axios.get(AppURL.ExternalprojectsListByStatusSA(status)).then(response=>{
            setprojects(response.data);
            setCountries(response.data);
            setFilteredCountries(response.data)
            //setLoading(false);
          })
          
          axios.get(AppURL.ExternalProjectManagerList).then(mresponse=>{
            setManagers(mresponse.data);
            console.log(mresponse.data)
          })
        }
        else{
          axios.get(AppURL.ExternalprojectsListByStatus(status,getexternal)).then(response=>{
            setprojects(response.data);
            setCountries(response.data);
            setFilteredCountries(response.data)
            //setLoading(false);
          })
        }
       

        

 
      },[])
        function preventBack(){ window.history.forward(); 
        
        }
        setTimeout(preventBack(),0);
        window.onunload = function () { 
          window.removeEventListener('popstate');
          window.removeEventListener('onunload')
         };
   
         const mainpage=(e)=>{
          e.preventDefault();
            navigate('/project-list');
             
        }
        

      const AllProjects=(e)=>{
        e.preventDefault();
        navigate('/active-projects'); 
        
         }

         const getCountries = async()=>{
          if(usercode=='SSA'){
            axios.get(AppURL.ExternalprojectsListAll).then(response=>{
              setprojects(response.data);
              //setLoading(false);
              setCountries(response.data);
              setFilteredCountries(response.data)
            })
          }
          else{
            axios.get(AppURL.ExternalprojectsList(getexternal)).then(response=>{
              setprojects(response.data);
              setCountries(response.data);
              setFilteredCountries(response.data)
              //setLoading(false);
            })
          }
      }
      

         useEffect(()=> {
          const result=countries.filter(project=>{
             // return project.project_name.toLowerCase().match(search.toLowerCase()) || project.project_id.toLowerCase().match(search.toLowerCase());
              return  project.project_id.toString().match(search.toLowerCase()) || project.project_name.toLowerCase().match(search.toLowerCase()) || project.project_area.toLowerCase().match(search.toLowerCase());
          });
          setFilteredCountries(result);
      },[search])
    const columns =[

      {
        name:"Project Refrence",
        selector:(row) =>
        row.phase>0?
        <>
        <span data-toggle="tooltip" data-placement="top" title={row.name}>{String(row.project_id).substring(4, String(row.project_id.length) - 1)}</span>-
        <span data-toggle="tooltip" data-placement="top" title={row.name}>{row.phase}</span>
        </>
        :row.project_id,
        sortable:true
    }  ,
    {
      name:"Project Title",
      selector:(row) => row.project_name,
      //sortable:true
  } , 
      {
          name:"Location",
          selector:(row) => row.project_area,
          sortable:true
      }  ,
     
      // {
      //     name:"Project Name",
      //     selector:(row) => <span data-toggle="tooltip" data-placement="top" title={row.name}>{row.name}</span>,
      //     sortable:true
      // }  ,  

     

      {
        name:"Status",
        cell:(row)=>
        <>

        <button className='btn btn-link'  onClick={(e)=>UpdateProjectStatus(e,row.project_id)}>{row.projectstatus}</button>


        </>
    },
      // {
      //     name:"Completion Date",
      //     selector:(row) => row.completed_at,
      //     sortable:true
      // }  ,  

      // {
      //     name:"Status",
      //     selector:(row) => row.status,
      //     sortable:true
      // }  ,  
      {
          name:"Action",
          cell:(row)=>
          <>

          <button className='btn btn-success'  onClick={(e)=>ProjectDetail(e,row.project_id,row.mainid)}>View Details</button>


          </>
      }
  ]



  function FilterPM(pm){
    setpmfilter(true)
    axios.get(AppURL.ExternalprojectsListBYPMStatus(status,pm)).then(response=>{
      setprojects(response.data);
      setCountries(response.data);
       setFilteredCountries(response.data)


    })
    
  }

  const UpdateProjectStatus=(e,projectId)=>{
    e.preventDefault();
    navigate('/project-status',{state:{id:projectId}});
  //alert('Not Available');
    //thisClicked.closest("tr").remove();
    //console.log(email);
     }


         const ProjectDetail=(e,projectId,main)=>{
          e.preventDefault();
          
          navigate('/single-project-details',{state:{id:projectId,mainid:main}});
        
          //thisClicked.closest("tr").remove();
          //console.log(email);
           }
    return (
      <Fragment>

 <div className='container-fluid dashboard-background-color mt-2'>
      
      <div className="content ">
   
   
        <Row>
                           
                                 
     <Col md="8">
                                   <h3 className=''> </h3>
                                   </Col>
                                   <Col md="4" className='mt-1 mb-2'>
                                   <button className='btn btn-danger'  onClick={(e)=> mainpage(e)} ><i className='fa fa-arrow-left'></i> Homepage</button>
                                   </Col>
                          {
                            usercode=='SSA'?
                            <Row className='mt-5'>
                            <Col md='2'><p>Filter By Project Manager:</p></Col>
                            <Col md='2'>
                              <select name='pm' onChange={(event) =>FilterPM(event.target.value) }>
                              <option>Select Project Manager</option> 
                                {
                                  managers.map((man,i)=>{
                                    return(
                                      <option value={man.projectmid}>{man.pmname}</option>
                                    )
                                    
                                  })
                                }
                                
                                
                              </select>
                            </Col>
                          </Row>
                            :null
                          }
                                   
          <Col md="12">
            <Card className=''>
              <>
                <DataTable 
                    columns={columns} 
                    data={filteredCountries} 
                    pagination title={ status+" Projects"} 
                    fixedHeader 
                    fixedHeaderScrollHeight="590px"
                    paginationRowsPerPageOptions={[10, 30, 50, 100]}
                    stateSave
                    stateDuration
                    selectableRowsHighlight
                    // expandableRows
                    highlightOnHover
                    subHeader
                    
                    subHeaderComponent={
                        <input 
                        type="text" 
                        placeholder='Search here' 
                        className='w-25 form-control' 
                            value={search}
                            onChange={(e)=> setSearch(e.target.value)}>
                        </input>
                    }
                ></DataTable>
                {/* <Table striped bordered hover>
                          <thead>
                            <tr>
                              <th>#</th>
                              <th>Refrence</th>
                              <th>Title</th>
                              <th>Location</th>
                              <th>Status</th>
                            </tr>
                          </thead>
                          <tbody> */}
                            {/* {
                              
                              projects.map((project,sr)=>{
                                return(
                             
                                )
                              })
                            } */}

                          {/* </tbody>
                        </Table> */}
              </>
              <Card.Body>
               
              </Card.Body>
              <Card.Footer>
                <hr />
                <div className="stats">
                  <i className="fa fa-history" /> Updated Just Now...
                </div>
              </Card.Footer>
            </Card>
          </Col>
        </Row>
      </div>
    </div>
      </Fragment>
    )
  
}

export default ProjectListByStatus