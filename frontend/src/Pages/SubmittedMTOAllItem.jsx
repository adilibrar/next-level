      ...
      const status=sessionStorage.getItem('code');
        ...
        axios.get(AppURL.SingleStockQTY).then(response=>{
          ...
          axios.get(AppURL.getProductionStock).then(response=>{
            ...
            axios.get(AppURL.ProjectAllMTOItems(MTOid)).then(response=>{
              setItem(response.data);
              axios.get(AppURL.ProjectAllIssuedItem(MTOid)).then(response=>{
...
if(loading){
   ...
   return(
        ...
        <Col sm={10}>
          <Tab.Content>
            ...
            <Tab.Pane eventKey="second">
             ...
             This is the complete list of MTO items that you added to the project.
        </Alert>
        ...
        item.map((single,i)=>{
          ...
          let reserved_qty=0
            ...
            //console.log(single.mto.projectmto['id']);
                    issued.map((q,ii)=>{
                      if ( q.Issued_item['id']==single.itemname['id'] && q.issuingmto['id']==single.mto['id']) {
                        reserved_qty=q.quantity
                    ...
                    <tr key={i}>
                       ...
                       {i+1}
                           ...
                           <td>{single.remarks}</td>
                            ...
                            {
                                reserved_qty==0?
                             <i placeholder='This item not reserved' className="fa-regular fa-circle btn btn-danger"></i>
                                :reserved_qty
                                }
                                </td>
                            </tr>
    ...
    <p className='mt-3'> <i placeholder='Items not reserved' className="fa-regular fa-circle btn btn-danger"></i> Nothing reserved for the items</p>
    </Table>
