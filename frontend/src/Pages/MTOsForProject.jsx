    ...
    if(e.target.description.value===''){
        ...
        axios.post(AppURL.AddMTO,data,{ 
            ...
            if(response.status===201){
                 cogoToast.success("MTO Created Sucessfully ...",{position:'top-right'});
                forceUpdate();
...
const getMTO = async()=>{
        try{
            const response= await axios.get(AppURL.MTOListByProject(ProjectID))
            setMTO(response.data);
            setfilteredMto(response.data)
        }catch(error){
            console.log(error);
    ...
    const columns =[
        ...
        {
            name:"Description",
            selector:(row) => <span data-toggle="tooltip" data-placement="top" title={row.description}>{row.description}</span>,
            sortable:true
        }  ,  
   ...
   return(
    ...
    <Tab.Container id="left-tabs-example" defaultActiveKey={tabstatus}>
      <Row>
        <Col sm={3}>
            ...
            <div className='col-md-12 row'>
                <div className='col-md-2'>
                  ...
                  <span className='form-inline input-group'>  
                    <select name="type" className='type-width'>
                        <option value="1">Profile</option>
                        <option value="2">Accessory</option>
                        <option value="3">Packing Material</option>
                        <option value="4">Gasket</option>
                        <option value="5">Screws</option>
                  ...
                  <span className='form-inline input-group'>  
                  <Form.Control type="number" placeholder="Margin Percentage" name='extra' defaultValue={0} />
                    </span>
                    </Form.Group>
