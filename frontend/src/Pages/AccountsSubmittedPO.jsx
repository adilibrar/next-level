...
const PODetail=(e,POId)=>{
        e.preventDefault();
      if(status=='AC'){
        navigate('/purchase-order-approval',{state:{POID:POId}});
      }
      else{
    ...
    const getProjects = async()=>{
        try{
            const response= await axios.get(AppURL.GETAccountsSubmittedPO)
      ...
      <Row>
        <Col sm={3}>
          <Nav variant="pills" className="flex-column">
            <Nav.Item>
              <Nav.Link eventKey="first">Request Purchase Orders</Nav.Link>
            </Nav.Item>
...
export default AccountsSubmittedPO
