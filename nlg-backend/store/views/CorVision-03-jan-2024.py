...
from store.models import Item,MTO,Project,WindowStatus,StockList,StockListItem,FinalReleasedWindowInterLock,FinalReleasedWindow,ReleasedCustomDim,MTOItem,MTOType,ReleasedWindow,FinalReleasedWindowProfile,ReleasedWindowProfile,ReleasedWindowACC,ReleasedWindowInterLockACC,ReleasedWindowGasket,ReleasedWindowScrew,ReleasedWindowPacking,ReleasedWindowGlass,ReleasedWindowInterLock,Floor,Lock,Elevation,CorVisionProfile,CORVISION,CorVisionInterLockAcc,CorVisionDIM,CorVisionShutter,CorVisionGlass,CorVisionAcc,CorVisionGasket,CorScrew,CorPacking,CorVisionInterLock
from store.Serializations.CorVisionSerializer import CorVisionProfileSerializer,FinalCorVisioninterProfileShortSerializer,FinalReleaseWindowInterProfileSerializer,CorVisionInterProfileShortSerializer,GetReleasedCustomDimSerializer,FinalCorVisionReleasedProfileShortSerializer,ReleasedCustomDimSerializer,FinalReleaseWindowProfileSerializer,ReleaseWindowShortSerializer,FinalReleaseWindowShortSerializer,CorVisionReleasedProfileShortSerializer,ReleaseWindowACCSerializer,ReleaseWindowSerializerDepth,ReleaseWindowPackingSerializer,ReleaseWindowGlassSerializer,ReleaseWindowGasketSerializer,ReleaseWindowScrewSerializer,ReleaseWindowInterlockACCSerializer,ReleaseWindowsaveSerializer,ReleaseWindowInterLockSerializer,ReleaseWindowProfileSerializer,ReleaseWindowSerializer,FloorSerializer,LockSerializer,ElevationSerializer,CorVisionInterLockAccSerializer,CorVisionInterLockSerializer,CorVisionProfileShortSerializer,CorVisionPackingSerializer,CorVisionScrewSerializer,CorVisionSerializer,CorVisionDIMSerializer,CorVisionShutterSerializer,CorVisionGlassSerializer,CorVisionAccSerializer,CorVisionGasketSerializer
...
def CORVision_Profile(request):
    ...
    if request.method == "POST":
        ...
        #     print("my status is with extra dim")
        ...
        # if(Corserializer.data['status']=='0'):
        #     #print("my status is with extra dim")
...
def ReleaseWindow(request):
    if request.method == "POST":
        ...
        # WindowStatus=Corvi.objects.get(id=int(requestdata['released']))
        # Wserializer = ReleaseWindowSerializer(WindowStatus)
...
def ReleaseWindowCustomDimData(request,pk):
    if request.method == "GET":
        ...
        # print(WindowData.values())
        Dimserializer = GetReleasedCustomDimSerializer(WindowData)
        print(Dimserializer.data)
        # ReleasedWindowCDim=pk
...
# @api_view(['GET'])
# @permission_classes([AllowAny])
# def ValidateStockList(request,pk):
#     if request.method == "GET":
#         WindowData = StockListItem.objects.filter(id=pk)
#         Lockserializer = Stocklistitems(WindowData)
